    ...
    public class ProductController : Controller
    {
        ...
        public IActionResult Upsert(ProductVM productVM)
        {
            if (!ModelState.IsValid)
            {
                var viewModel = new ProductVM()
                {
                    Product = productVM.Product.Id == 0? productVM.Product : _unitOfWork.Product.Get(productVM.Product.Id),
                    CategoryList = _unitOfWork.Category.GetAll().Select(m => new SelectListItem()
                    {
                        Text = m.Name,
                        Value = m.Id.ToString()
        ...
        {
            var obj = _unitOfWork.Product.GetAll(includeProperties: "Category,CoverType");
